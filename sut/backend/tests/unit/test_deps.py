...
class TestGetUserContext:
    ...
    async def test_raises_401_for_missing_sub_claim(self):
        ...
        mock_payload = {"email": "user@example.com"}  # No sub
...
class TestFindOrCreateUser:
    ...
    def test_handles_race_condition_on_create(self, mock_db_session, mock_user):
        ...
        # First query returns None, then after IntegrityError returns user
...
class TestSyncUserClaims:
    ...
    def test_updates_changed_claims(self, mock_db_session, mock_user):
        ...
        ctx = UserContext(
            ...
            full_name="Updated Name",  # Different from mock_user
            is_admin=True,  # Different from mock_user
        )
    ...
    def test_no_update_when_claims_match(self, mock_db_session, mock_user):
        ...
        ctx = UserContext(
            auth_service_id=123,
            email="test@example.com",
            full_name="Test User",  # Same as mock_user
            is_admin=False,  # Same as mock_user
        )
    ...
    def test_updates_auth_service_id(self, mock_db_session, mock_user):
        ...
        mock_user.auth_service_id = 100  # Different from ctx
        ctx = UserContext(
            auth_service_id=200,
            email="test@example.com",
            full_name="Test User",
