...
class TestDecodeToken:
    ...
    def test_decode_valid_token(self, rsa_key_pair, valid_token_payload):
        """decode_token returns payload for valid token."""
        token = create_signed_token(valid_token_payload, rsa_key_pair["private"])
        
        with patch("app.core.security.get_public_key", return_value=rsa_key_pair["public"]):
    ...
    def test_decode_expired_token_returns_none(self, rsa_key_pair, expired_token_payload):
    ...
    def test_decode_wrong_issuer_returns_none(self, rsa_key_pair, valid_token_payload):
    ...
    def test_decode_invalid_signature_returns_none(self, rsa_key_pair, valid_token_payload):
        ...
        wrong_private_key = rsa_gen.generate_private_key(public_exponent=65537, key_size=2048)
    ...
    def test_decode_malformed_token_returns_none(self, rsa_key_pair):
    ...
    def test_decode_empty_token_returns_none(self, rsa_key_pair):
    ...
    def test_decode_token_missing_exp_returns_none(self, rsa_key_pair):
...
class TestGetPublicKey:
    ...
    def test_env_var_ignores_force_refresh(self, rsa_key_pair):
...
class TestFetchPublicKey:
    ...
    def test_fetches_from_correct_endpoint(self):
        """_fetch_public_key calls auth service /internal/public-key endpoint."""
        mock_response = MagicMock()
        mock_response.json.return_value = {"public_key": "test-key"}
        
        with patch("app.core.security.httpx.get", return_value=mock_response) as mock_get:
            result = security._fetch_public_key()
        
        assert mock_get.called
        call_args = mock_get.call_args
        assert "/internal/public-key" in call_args[0][0]
    ...
    def test_raises_on_http_error(self):
        ...
        with patch("app.core.security.httpx.get", side_effect=httpx.HTTPStatusError(
